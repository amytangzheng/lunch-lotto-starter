...
async function fetchRestaurants() {
  try {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      // Foursquare API URL for searching nearby restaurants
      const url = `https://api.foursquare.com/v3/places/search?ll=${lat},${lng}&radius=${milesToMeters(settings.distance)}&query=restaurant&categories=food&limit=10`;

      const response = await fetch(url, {
        headers: {
          'Authorization': `Bearer YOUR_FOURSQUARE_API_KEY`,  // Use your actual API key here
        },
      });
      ...
      // Wait for 2 seconds before showing the wheel
      setTimeout(() => {
